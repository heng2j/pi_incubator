...
_original_signature_from_callable = inspect._signature_from_callable

# Define a patched version that ignores GenericAlias errors
def patched_signature_from_callable(obj, *args, **kwargs):
    try:
        return _original_signature_from_callable(obj, *args, **kwargs)
    except ValueError as e:
        if "no signature found for builtin type" in str(e) and isinstance(obj, GenericAlias):
            return inspect.Signature()
        raise e

# Monkey patch the function
inspect._signature_from_callable = patched_signature_from_callable
...
if __name__ == "__main__":
    ...
    policy_module = ProbabilisticActor(
        ...
        distribution_kwargs={
            "min": env.action_spec.space.low, #env.action_spec_unbatched.space.low,
            "max": env.action_spec.space.high, # env.action_spec_unbatched.space.high,
        },
    ...
    ray_init_config["local_mode"] = True

    import inspect
    SyncDataCollector.__signature__ = inspect.Signature()


    # TODO - I need to create my own RayCollector with the latest ray
    ...
    exp_name = f"ppo_exp_{uuid.uuid1()}"
